...
logging.basicConfig(filename=os.getcwd() + '\quickbooks_dump.log', level=logging.INFO, format='%(asctime) %(levelname)s : %(message)s')
...
find_click('id', 'DEPOSIT_DETAIL_reportListLink_Banking', '', 10, "Getting Deposit Details")
...
find_click('id', 'JOURNAL_reportListLink_Accountant & Taxes', '', 10, "Getting Journal report...")
driver.switch_to_default_content()
home_frames = driver.find_elements_by_tag_name('iframe')
driver.switch_to_frame(home_frames[0]) # since the page was reloaded
date_macro = driver.find_element_by_id('date_macro')
